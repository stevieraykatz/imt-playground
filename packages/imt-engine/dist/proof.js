 ...
 */
import { getNodeByKey, findPredecessor, keyExists } from './engine.js';
import { hashNode, hashPair, hashRootWithSize } from './hash.js';
/**
 * Generate a Merkle path (siblings and path indices) for a leaf at the given index
 */
export function getMerklePath(state, leafIndex) {
    const siblings = [];
    const pathIndices = [];
 ...
 */
export function generateProof(state, key) {
    // Check if key exists
 ...
 */
export function generateInclusionProof(state, key) {
 ...
 */
export function generateExclusionProof(state, key) {
 ...
 */
export function verifyInclusionProof(proof, expectedRoot, size) {
